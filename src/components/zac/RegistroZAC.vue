...
import LineaZAC from './LineaZAC.vue';
import CuchillosZAC from './CuchillosZAC.vue';
...
const isAllValid = computed(() => {
  return supervisor.value.trim() !== '' && 
         responsableBandejas.value.trim() !== '' &&
         linea1Valid.value && 
         linea2Valid.value && 
         linea3Valid.value && 
...
async function generateAndSendPDF() {
  ...
  let yPos = 15;
  pdf.setFontSize(20);
  pdf.text("Reporte de Registro ZAC", 105, yPos, { align: 'center' });
  yPos += 15;
  pdf.setFontSize(12);
  ...
  if (cuchillosRef.value) {
    ...
    pdf.text("Cuchillos y Lavado de Bandejas", 14, yPos);
    yPos += 7;
    pdf.line(14, yPos - 5, 196, yPos - 5);
    autoTable(pdf, { startY: yPos, theme: 'plain', body: [['Responsable de Bandejas:', responsableBandejas.value], ['¿Se han usado cuchillos?:', cuchillosData.usados ? 'Sí' : 'No'], ...(cuchillosData.usados ? [['Detalles:', cuchillosData.detalles]] : [])] });
    yPos = pdf.lastAutoTable.finalY + 15;
  }
  ...
  if (lineasData.length > 0) {
    ...
    yPos += 7;
    pdf.line(14, yPos - 5, 196, yPos - 5);
    autoTable(pdf, { startY: yPos, head: [['Línea', 'Producto', 'N. Controladora', 'N. Maquinista', 'Peso Medio', 'Test Metales']], body: lineasData.map(d => [d.numero, d.producto, d.controladora, d.real, d.pesoMedio, d.metalesOK ? 'Sí' : 'No']) });
  }
  
  const base64PDF = pdf.output('datauristring');
  const pdfContent = base64PDF.split(',')[1];
  const fileName = `Registro_ZAC_${supervisor.value.replace(/ /g, '_')}_${props.recordDate}.pdf`;
...
<template>
  ...
  <div class="container mx-auto p-4 md:p-8 pb-24">
    ...
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-lg">
      <h1 class="text-3xl md:text-4xl font-bold text-center text-gray-800 mb-6">Registro ZAC</h1>
      <div class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div class="flex flex-col">
            <label class="font-medium text-gray-700 mb-1">Nombre del supervisor:</label>
            <input type="text" v-model="supervisor" class="p-2 border border-gray-300 rounded-md" />
          </div>
          <div class="flex flex-col">
            <label class="font-medium text-gray-700 mb-1">Fecha del registro:</label>
            <input type="date" :value="recordDate" @input="updateDate" class="p-2 border border-gray-300 rounded-md" />
          </div>
          <div class="flex flex-col">
        ...
        <div v-else>
          <LineaZAC ref="linea1Ref" :number="1" :allProducts="allProducts" @validate="isValid => linea1Valid = isValid" />
          <LineaZAC ref="linea2Ref" :number="2" :allProducts="allProducts" @validate="isValid => linea2Valid = isValid" />
          <LineaZAC ref="linea3Ref" :number="3" :allProducts="allProducts" @validate="isValid => linea3Valid = isValid" />
          <CuchillosZAC ref="cuchillosRef" v-model:responsable="responsableBandejas" @validate="isValid => cuchillosValid = isValid" />
        </div>
      </div>
    </div>
